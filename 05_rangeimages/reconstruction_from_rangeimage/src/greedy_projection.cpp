...
int main (int argc, char** argv) {
	...
	parse_argument (argc, argv, "-cx", cx);		// 光轴在深度图像上的 x 坐标
	parse_argument (argc, argv, "-cy", cy);		// 光轴在深度图像上的 y 坐标
	parse_argument (argc, argv, "-fx", fx);		// 水平方向焦距
	parse_argument (argc, argv, "-fy", fy);		// 垂直方向焦距
	parse_argument (argc, argv, "-type", type);	// 重建曲面时三角化的方式
	parse_argument (argc, argv, "-size", size);	// 重建曲面时面片的大小
	...
	sensorPose.setIdentity();	// 将模型视图矩阵初始化为单位矩阵
	pcl::RangeImage::CoordinateFrame coordinate_frame = pcl::RangeImage::CAMERA_FRAME;
	float noiseLevel = 0.00;	// 成像时模拟噪声的水平
	float minRange = 0.0f;		// 成像时考虑该阈值以外的点
	...
	pcl::visualization::RangeImageVisualizer range_image_widget ("PCL-Book tutorial");
	range_image_widget.showRangeImage (*rangeImage);
	range_image_widget.setWindowTitle("PCL-Book tutorial");
	...
	boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer (new pcl::visualization::PCLVisualizer ("PCL-Book tutorial"));
